...
当前工作流提供发布附件和元数据。图片直粘 1.4.0 支持托盘手动检查正式版本，用户确认后自动下载、校验、替换 EXE 并启动。1.3.x 需要手动安装支持自动更新的版本一次。不定时自动检查。

客户端按 `<tool-id>/v` 筛选正式 Release，不能使用仓库全局 latest 作为每个工具的版本来源。先过滤工具、草稿和预发布，再按数字版本比较，不按字符串排序。图片直粘按固定仓库、版本和附件名构造下载 URL，读取对应 ZIP 和 `.sha256`；只接受 HTTPS GitHub 发布附件来源，验证校验和、包内文件白名单及 EXE 版本。

元数据包含工具 ID、版本、目标平台、下载链接、文件大小和 SHA-256。哈希用于完整性校验，不是独立的代码签名；图片直粘的信任来源是固定官方仓库的 HTTPS 发布附件。只有可信维护者应有发布权限。
